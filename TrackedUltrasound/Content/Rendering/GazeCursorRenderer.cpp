...
namespace TrackedUltrasound
{
  ...
  void GazeCursorRenderer::Update( const DX::StepTimer& timer, SpatialPointerPose^ pointerPose )
  {
    ...
    // and set that as the models internal coordinate system (m_modelMatrix)
  ...
  void GazeCursorRenderer::CreateDeviceDependentResources()
  {
    ...
    auto loadModelDataTask = loadShadersTask.then([this, meshDataVector]()
    {
      auto storageTask = create_task( StorageFile::GetFileFromApplicationUriAsync( ref new Windows::Foundation::Uri( "ms-appx:///Assets/model.cmo" ) ) );
      auto storageFile = storageTask.get();
    ...
    auto createModelTask = loadModelDataTask.then( [this]( task<std::shared_ptr<std::vector<uint8_t>>> previousTask )
    {
